module.exports = {
  apps: [
    {
      name: 'backend',
      script: 'entrypoint.sh',
      interpreter: 'bash',
      cwd: '/home/ThePredMain/backend',
      instances: 1,
      autorestart: true,
      watch: false,
      max_memory_restart: '1G',
      env: {
        PYTHONPATH: '/home/ThePredMain/backend',
        POSTGRES_HOST: 'localhost',
        POSTGRES_PORT: 5432,
        POSTGRES_DB: 'thepred',
        POSTGRES_USER: 'thepred',
        POSTGRES_PASSWORD: 'SUPER_STRONG_PASSWORD_CHANGE_ME_123!@#',
        REDIS_HOST: 'localhost',
        REDIS_PORT: 6379,
        JWT_SECRET: 'YOUR_SUPER_SECRET_JWT_KEY_MINIMUM_32_CHARS_RANDOM',
        JWT_ALGORITHM: 'HS256',
        JWT_EXPIRE_HOURS: 24,
        DEBUG: 'false',
        INITIAL_PRED_BALANCE: 10000,
        REFERRAL_BONUS_PRED: 1000,
        BOT_TOKEN: '8067436515:AAGHg6_ojgsnBmREI1U9Sr_iibgXYGInml0',
        WEBAPP_URL: 'https://thepred.tech',
        S3_ENDPOINT: 'http://localhost:9000',
        S3_ACCESS_KEY: 'admin',
        S3_SECRET_KEY: 'Ivanbunin110818',
        S3_BUCKET: 'thepred-events',
        S3_PUBLIC_URL: 'https://thepred.store',
      },
    },
    {
      name: 'webapp',
      script: 'main.py',
      cwd: '/home/ThePredMain/webapp',
      interpreter: '/home/ThePredMain/venv/bin/python3',
      instances: 1,
      autorestart: true,
      watch: false,
      max_memory_restart: '512M',
      env: {
        PYTHONPATH: '/home/ThePredMain/webapp',
        API_URL: 'http://localhost:8000',
        WEBAPP_URL: 'https://thepred.tech',
        WEBAPP_SECRET_KEY: 'WEBAPP_SECRET_KEY_RANDOM_STRING_32_CHARS',
        DEV_MODE: 'false',
        BOT_TOKEN: '8067436515:AAGHg6_ojgsnBmREI1U9Sr_iibgXYGInml0',
        BOT_USERNAME: 'The_Pred_Bot',
        S3_PUBLIC_URL: 'https://thepred.store',
        S3_BUCKET: 'thepred-events',
      },
    },
    {
      name: 'admin',
      script: 'main.py',
      cwd: '/home/ThePredMain/admin',
      interpreter: '/home/ThePredMain/venv/bin/python3',
      instances: 1,
      autorestart: true,
      watch: false,
      max_memory_restart: '512M',
      env: {
        PYTHONPATH: '/home/ThePredMain/admin',
        API_URL: 'http://localhost:8000',
        ADMIN_PASSWORD: 'STRONG_ADMIN_PASSWORD_CHANGE_ME',
        ADMIN_SECRET_KEY: 'ADMIN_SECRET_KEY_RANDOM_32_CHARS',
        S3_PUBLIC_URL: 'https://thepred.store',
        S3_BUCKET: 'thepred-events',
      },
    },
    {
      name: 'bot',
      script: 'main.py',
      cwd: '/home/ThePredMain/bot',
      interpreter: '/home/ThePredMain/venv/bin/python3',
      instances: 1,
      autorestart: true,
      watch: false,
      max_memory_restart: '256M',
      env: {
        PYTHONPATH: '/home/ThePredMain/bot',
        BOT_TOKEN: '8067436515:AAGHg6_ojgsnBmREI1U9Sr_iibgXYGInml0',
        API_URL: 'http://localhost:8000',
        WEBAPP_URL: 'https://thepred.tech',
      },
    },
    {
      name: 'landing',
      script: 'main.py',
      cwd: '/home/ThePredMain/landing',
      interpreter: '/home/ThePredMain/venv/bin/python3',
      instances: 1,
      autorestart: true,
      watch: false,
      max_memory_restart: '256M',
      env: {
        PYTHONPATH: '/home/ThePredMain/landing',
      },
    },
    {
      name: 'telegram-worker',
      script: 'telegram_worker.py',
      cwd: '/home/ThePredMain/backend',
      interpreter: '/home/ThePredMain/venv/bin/python3',
      instances: 1,
      autorestart: true,
      watch: false,
      max_memory_restart: '512M',
      env: {
        PYTHONPATH: '/home/ThePredMain/backend',
        POSTGRES_HOST: 'localhost',
        POSTGRES_PORT: 5432,
        POSTGRES_DB: 'thepred',
        POSTGRES_USER: 'thepred',
        POSTGRES_PASSWORD: 'SUPER_STRONG_PASSWORD_CHANGE_ME_123!@#',
        REDIS_HOST: 'localhost',
        REDIS_PORT: 6379,
        BOT_TOKEN: '8067436515:AAGHg6_ojgsnBmREI1U9Sr_iibgXYGInml0',
      },
    },
    {
      name: 'broadcast-scheduler',
      script: 'broadcast_scheduler.py',
      cwd: '/home/ThePredMain/backend',
      interpreter: '/home/ThePredMain/venv/bin/python3',
      instances: 1,
      autorestart: true,
      watch: false,
      max_memory_restart: '256M',
      env: {
        PYTHONPATH: '/home/ThePredMain/backend',
        POSTGRES_HOST: 'localhost',
        POSTGRES_PORT: 5432,
        POSTGRES_DB: 'thepred',
        POSTGRES_USER: 'thepred',
        POSTGRES_PASSWORD: 'SUPER_STRONG_PASSWORD_CHANGE_ME_123!@#',
        REDIS_HOST: 'localhost',
        REDIS_PORT: 6379,
      },
    },
    {
      name: 'leaderboard-scheduler',
      script: 'leaderboard_scheduler.py',
      cwd: '/home/ThePredMain/backend',
      interpreter: '/home/ThePredMain/venv/bin/python3',
      instances: 1,
      autorestart: true,
      watch: false,
      max_memory_restart: '256M',
      env: {
        PYTHONPATH: '/home/ThePredMain/backend',
        POSTGRES_HOST: 'localhost',
        POSTGRES_PORT: 5432,
        POSTGRES_DB: 'thepred',
        POSTGRES_USER: 'thepred',
        POSTGRES_PASSWORD: 'SUPER_STRONG_PASSWORD_CHANGE_ME_123!@#',
        REDIS_HOST: 'localhost',
        REDIS_PORT: 6379,
      },
    },
  ],
};
